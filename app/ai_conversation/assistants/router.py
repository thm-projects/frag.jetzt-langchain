...
from app.ai_conversation.assistants.models import InputAssistant, OutputAssistant
...
async def get_user_assistants(request: Request) -> list[OutputAssistant]:
    try:
        config = {"configurable": {}}
...
async def patch_user_assistant(request: Request, assistant_id: UUID, assistant: dict):
    try:
        config = {"configurable": {}}
        await per_req_config_modifier(config, request)
...
@router.delete(
    "/assistant/{assistant_id}/file/{file_id}",
    dependencies=DEPENDENCIES,
    tags=["Assistant (User) - Files"],
)
async def delete_user_assistant_file(
    request: Request, assistant_id: UUID, file_id: UUID
):
    try:
        config = {"configurable": {}}
        await per_req_config_modifier(config, request)
        return await file_u_delete(config, assistant_id, file_id)
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_room_assistants(request: Request) -> list[OutputAssistant]:
    try:
        config = {"configurable": {}}
        await per_req_config_modifier(config, request)
...
async def patch_room_assistant(request: Request, assistant_id: UUID, assistant: dict):
    try:
        config = {"configurable": {}}
        await per_req_config_modifier(config, request)
...
@router.delete(
    "/room-assistant/{assistant_id}/file/{file_id}",
    dependencies=ROOM_DEPENDENCIES,
    tags=["Assistant (Room) - Files"],
)
async def delete_room_assistant_file(
    ...
    try:
        config = {"configurable": {}}
        await per_req_config_modifier(config, request)
        return await file_r_delete(config, assistant_id, file_id)
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_platform_assistants(request: Request) -> list[OutputAssistant]:
    try:
        config = {"configurable": {}}
        await per_req_config_modifier(config, request)
...
async def patch_platform_assistant(
    request: Request, assistant_id: UUID, assistant: dict
):
    try:
        config = {"configurable": {}}
        await per_req_config_modifier(config, request)
...
@router.delete(
    "/platform-assistant/{assistant_id}/file/{file_id}",
    dependencies=DEPENDENCIES,
    tags=["Assistant (Platform) - Files"],
)
async def delete_platform_assistant_file(
    ...
    try:
        config = {"configurable": {}}
        await per_req_config_modifier(config, request)
        return await file_p_delete(config, assistant_id, file_id)
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
